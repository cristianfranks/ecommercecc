...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta en nuestra Tienda Virtual C&C Logistica de Limpieza'
...
def logout(request):
    auth.logout(request)
    messages.success(request, 'Salio de Sesion')
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Felicidades, su cuenta esta activa')
        return redirect ('login')
    else:
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Un Email fue enviado a su bandeja de entrada para resetear su contraseña')
            return redirect('login')
        else:
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            messages.success(request, 'La contraseña de confirmacion no concuerda')
            return redirect('resetPassword')
    else:
